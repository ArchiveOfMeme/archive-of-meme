...
export default function MemeCard({ meme }) {
  ...
  const handleTwitterShare = (e) => {
    ...
    const siteUrl = typeof window !== 'undefined'
      ...
      : `https://archiveofmeme.fun/meme/${meme.id}`;
    ...
    <article className="overflow-hidden border-b border-[#2a2a2a]">
      ...
      <div className="p-4 pt-2">
        <div className="flex items-center justify-between">
          {/* Left: Engagement icons */}
          <div className="flex items-center gap-1">
            {/* Comments */}
            <Link
              href={`/meme/${meme.id}`}
              className="flex items-center gap-1.5 p-2 text-[#71767b] hover:text-[#1d9bf0] hover:bg-[#1d9bf0]/10 rounded-full transition-colors"
          ...
          <a
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 bg-gradient-to-r from-[#2081e2] to-[#1868b7] text-white text-xs font-semibold rounded-full hover:from-[#1868b7] hover:to-[#145a9e] transition-all shadow-md shadow-[#2081e2]/20 hover:shadow-[#2081e2]/40"
          >
