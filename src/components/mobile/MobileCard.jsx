...
/**
 * MobileCard - Card de meme optimizada para móvil
 *
 * Características:
 * - Imagen full-width
 * - Acciones grandes para touch
...
export default function MobileCard({ meme }) {
  ...
  const handleShare = (e) => {
    ...
    // Usar Web Share API si está disponible (mejor para móvil)
    if (navigator.share) {
      navigator.share({
        title: meme.name,
        text: text,
        ...
        // Fallback a Twitter si el usuario cancela
      });
    } else {
      // Fallback a Twitter
      window.open(
  ...
  return (
    <article className="bg-[var(--bg-primary)] border-b border-[var(--bg-elevated)]">
      {/* Header con nombre */}
      <div className="px-4 py-3">
        <Link href={`/meme/${memeSlug}`}>
          <h2 className="text-white font-semibold text-lg">{meme.name}</h2>
        </Link>
        {formattedDate && (
      ...
      {/* Imagen - Full width, clickable */}
      <Link href={`/meme/${memeSlug}`} className="block">
        <Image
          src={meme.image}
          alt={meme.name}
          width={600}
      ...
      {/* Barra de acciones - Touch friendly */}
      <div className="px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-1">
          {/* Comentarios */}
          <Link
            href={`/meme/${memeSlug}`}
            className="flex items-center gap-2 px-3 py-2 text-[var(--text-muted)] hover:text-[var(--accent-blue)] hover:bg-[var(--accent-blue)]/10 rounded-full transition-colors"
          >
          ...
          <button
            onClick={handleShare}
            className="flex items-center gap-2 px-3 py-2 text-[var(--text-muted)] hover:text-[var(--accent-primary)] hover:bg-[var(--accent-primary)]/10 rounded-full transition-colors"
          >
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <a
          ...
          className="flex items-center gap-2 px-4 py-2 bg-[var(--accent-blue)] text-white text-sm font-semibold rounded-full hover:bg-[var(--accent-blue)]/80 transition-all"
        >
      ...
      {meme.description && (
        <div className="px-4 pb-4">
          <p
            className={`text-[var(--text-secondary)] text-sm whitespace-pre-line ${
          ...
          {meme.description.length > 100 && (
            <button
              ...
              className="text-[var(--accent-primary)] text-sm mt-1"
            >
